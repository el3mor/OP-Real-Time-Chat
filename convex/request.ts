...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const requsetAlreadySent = await ctx.db.query("requests").withIndex("by_receiver_sender", q => q.eq("receiver", receiver._id).eq("sender", currentUser._id)).unique() 

    if(requsetAlreadySent){
      throw new ConvexError("Request already sent")
    }

    const requsetAlreadyReceiver = await ctx.db.query("requests").withIndex("by_receiver_sender", q => q.eq("receiver", currentUser._id).eq("sender", receiver._id)).unique()

    if(requsetAlreadyReceiver){
    ...
    if(friends1.some(f => f.user2 === receiver._id) || friends2.some(f => f.user1 === receiver._id)){
